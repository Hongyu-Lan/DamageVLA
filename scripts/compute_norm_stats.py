...
def main(config_name: str, max_frames: int | None = None):
    ...
    keys = ["state", "actions"]
    # Force-aware configs (ForceVLA) add a normalized "force" modality; gated so other datasets
    # (which do not produce a "force" key) are unaffected.
    if getattr(config.model, "force_aware", False):
        keys.append("force")
