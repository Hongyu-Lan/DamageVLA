#!/usr/bin/env python3
"""Independently recompute and validate the complete 0911 post-processing."""
...
from apply_image_crop_20260911 import DEFAULT_RGB, DEFAULT_WRIST, LAYOUT_NOTE, SIZE
...
def main() -> int:
    ...
    errors = []
    stage_counts, fruits = Counter(), Counter()
    valid = masked = 0
    ...
    try:
        ...
        if primary_meta["n_group"] != 16 or primary_meta["k"] != 4:
        ...
        group_index = {
            (g["task"], g["fruit"], g["stage"]): g["group_id"]
    ...
    for path in paths:
        try:
            ...
            raw = load_jsonl(path.with_name(path.name + ".before_stage_20260911"))
            ...
            if group_index is not None:
                for index, record in enumerate(records):
                    if record["stage"] in SUPERVISED_STAGES:
                        group_id = group_index[(TASK, fruit, record["stage"])]
                        if (
                            record["group_id"] != group_id
                            or record["group_task"] != TASK
                            or record["group_fruit"] != fruit
