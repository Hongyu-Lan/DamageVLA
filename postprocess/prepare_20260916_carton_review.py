#!/usr/bin/env python3
"""Create read-only crop and outcome review sheets for the 2026-09-16 carton batch.
...
def episode_info(
    episode: Path,
) -> tuple[list[dict], str, str, list[tuple[str, int, int, int]]]:
    path = episode / "observations.jsonl"
    records = load_jsonl(path)
    object_category, load_condition, _ = physical_condition(records[0], path)
    stages, _ = compute_ordered_stages(records, path)
    return records, object_category, load_condition, summarize_segments(stages)
...
def representative_episodes(episodes: list[Path]) -> list[Path]:
    ...
    for episode in episodes:
        ...
        records = load_jsonl(path)
        object_category, load_condition, _ = physical_condition(records[0], path)
        grouped.setdefault((object_category, load_condition), []).append(episode)
    selected = []
    ...
    for camera, candidates in CANDIDATES.items():
        ...
        for name, box in unique_candidates.items():
            ...
            for episode in selected:
                records, object_category, load_condition, segments = episode_info(episode)
                for stage, start, end, _ in segments:
                    ...
                    tag = f"{object_category}/{load_condition} {episode.name[-6:]} {stage} f={index}"
    ...
    for episode in representative_episodes(episodes):
        ...
        for stage in ("prepare", "grasp", "place"):
            ...
            for name, box in candidates.items():
                ...
                images.append(label(crop, f"{object_category}/{load_condition} {episode.name[-6:]} {stage} | {name}"))
...
def end_contact_sheet(episodes: list[Path], output: Path) -> None:
    ...
    closeups = []
    for episode in episodes:
        records, object_category, load_condition, _ = episode_info(episode)
        index = len(records) - 1
        image = source_image(episode, records[index], "rgb")
        images.append(label(image, f"{episode.name[-6:]} {object_category}/{load_condition} end f={index}"))
        closeups.append(
            label(
                image.crop((720, 520, 1320, 1080)),
                f"{episode.name[-6:]} {object_category}/{load_condition} end f={index}",
            )
        )
...
def force_contact_review(episodes: list[Path], output: Path) -> None:
    ...
    for camera in ("rgb", "wrist"):
        ...
        for episode, records, peak in flagged:
            stages, _ = compute_ordered_stages(
                records, episode / "observations.jsonl"
            )
            reset = stages.index("reset")
...
def main() -> int:
    ...
    output = args.output_dir or args.root / "review_20260916_carton"
