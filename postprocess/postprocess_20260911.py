#!/usr/bin/env python3
"""Non-image post-processing for the 2026-09-11 DamageVLA batch.
...
soft prototypes, and per-episode initial-static flange F/T zeroing.  Camera
images and image metadata are deliberately outside this script.
"""
...
def build_prototypes(
    ...
    pooled: dict[tuple[str, str, str], list[list[float]]] = {}
    contributors: dict[tuple[str, str, str], set[str]] = {}
    used = []
    for path in paths:
        if path.parent.name in excluded:
            continue
        records = load_jsonl(path)
        fruit = fruit_from_prompt(str(records[0].get("prompt", "")), path)
        used.append(path.parent.name)
        for index, record in enumerate(records):
            stage = record.get("stage")
            if stage not in SUPERVISED_STAGES:
                continue
            key = (TASK, fruit, stage)
            pooled.setdefault(key, []).append(finger_mean_wrench(record, path, index))
            contributors.setdefault(key, set()).add(path.parent.name)
    keys = sorted(pooled, key=lambda key: (key[1], STAGES.index(key[2])))
...
def add_prototype_labels(records: list[dict], path: Path, bundle: dict) -> tuple[int, int]:
    ...
    group_index = {
        (group["task"], group["fruit"], group["stage"]): group["group_id"]
    ...
    for record in records:
        ...
        if stage in SUPERVISED_STAGES:
            group_id = group_index[(TASK, fruit, stage)]
            record.update({
                "group_id": group_id,
                "group_task": TASK,
                "group_fruit": fruit,
...
def main() -> int:
    ...
    for path in paths:
        ...
        if args.write:
            backup_once(path, ".before_stage_20260911")
