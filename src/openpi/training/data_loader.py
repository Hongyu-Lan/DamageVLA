...
class DataLoaderImpl(DataLoader):
    ...
    def __iter__(self):
        for batch in self._data_loader:
            # `aux` is empty for every dataset that does not carry these keys, which keeps the
            # 3-tuple a no-op for every pre-existing config.
            aux = {k: batch[k] for k in AUX_KEYS if k in batch}
            yield _model.Observation.from_dict(batch), batch["actions"], aux
