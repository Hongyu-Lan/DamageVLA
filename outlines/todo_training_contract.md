...
| **K** | **K=6（已定，2026-09-16）**。silhouette 在 K=4/5/6 完全持平（0.397–0.399）、稳定性都接近 1，两个指标选不出 K，按可解释性选：K=6 是最小的能让**满盒的持握单独成簇**的 K。K=4 也可接受且与现配置一致 | 见分析文档第 5 节，扫描表要进论文附录 |
...
| 条件 | episode | mu | 处理 |
|---|---|---|---|
| carton_full | `pi0_train_20260916_015938_carton_02_full` | 4.43 | **排除**（两条规则都命中：3xMAD 离群，且 25/25 taxel 饱和、闭合到 19.5 mm 而其他满盒 31.5 mm 以上）。它是该 session 的第一条，后面九条在 0.29–0.71 之间且无趋势，属于一次性热身失误，不是系统漂移。文件保留，可作为过紧抓取的失败案例 |
...
`physical.py` 的 `PhysicalActionProjector` 输出上乘一个标量参数：

```python
self.gain = nnx.Param(jnp.array(INIT_GAIN))      # 可学习
...
return self.gain.value * x.reshape(...)
```

初始化数值由实测反推：当前 `g_fvl_rms ≈ 2.0`、`g_phy_rms ≈ 0.045`，比值 0.023。
要把比值抬到 0.3 需要放大约 13 倍，**已定 `INIT_GAIN = 13`**（2026-09-16）。
若前 500 步 `loss_flow` 明显差于基线的 0.028，再降到 5–8。

每步日志里除了 `g_phy_rel`，把 `alpha` 本身也记下来。

**注意 alpha 必须可学习，不能写死一个大值。** 写死等于强行让物理引导影响动作，
如果这样"调"出好结果，那是在调参不是在验证。可学习的意义就是让数据回答。

**早期风险**：初始化放大后，前几百步 `z_phy` 还没学好，大幅度的 G_phy 可能拖累 `loss_flow`。
对照上一次的基线（step 0–650 时 `loss_flow ≈ 0.028`）：如果新的跑法在前 500 步明显更差，
把 `INIT_GAIN` 降到 5–8 再试。

### 如果 alpha 最后被压回接近 0，说明什么

这是必须提前想好的情况，对应论文里三个不同的退路，**用三个 arm 的对比来区分**：

| 观察到的结果 | 含义 | 论文怎么写 |
|---|---|---|
| alpha 保持或增大，full 优于 no-guidance | 直接引导通路有效 | 按现在的写法，机制成立 |
| alpha 变小，但 full ≈ no-guidance 且都优于 architecture-only | 物理监督有效，但作用是**塑造共享的 FVLMoE 表征**，不是靠直接通路（辅助损失仍通过 G_fvl 影响动作） | 方法叙述改成"辅助物理监督改善了力感知 VLA 的表征"，删掉引导通路这一机制主张 |
| architecture-only ≈ full | 物理监督整体没用 | 诚实的负面结果。文章的支点只能移到数据集与分析（外观相同但物理不同的任务设计、力的必要性论证），方法部分要大改 |

**所以这三个 arm 必须都跑，而且要在真机之前先用离线指标看一眼趋势。**

**训练时的头号监控指标就是 `g_phy_rel` 和 `alpha`**：跑完先看它们。如果 alpha 没被压小而 `g_phy_rel`
仍然很低，说明实现有问题；如果 alpha 被压小，按上表的第二、三行准备。
